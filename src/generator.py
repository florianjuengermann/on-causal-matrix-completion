...
def getRatingAndPropensityMatrix(inv_scale=1, alpha=0.01, r=5, seed=0):
    ...
    # users
    core_user_matrix = rng.normal(size=(num_core_users, latent_dim))
    dirichlet_alphas = [alpha]*num_core_users
    B_transform_matrix = rng.dirichlet(
    ...
    # movies
    core_movie_matrix = rng.normal(size=(num_core_movies, latent_dim))
...
def getRatingAndPropensityMatrix_general(latent_movie_matrix=None, inv_scale=1, alpha=0.01, r=5, seed=0):
    ...
    # users
    latent_user_matrix = rng.normal(size=(num_users, latent_dim))

    # movies
    if latent_movie_matrix is None:
        core_movie_matrix = rng.normal(
